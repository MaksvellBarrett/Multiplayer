...
public class CustomHandManager : OVRCameraRig, IPunObservable
{
	public PhotonView HandView;
	...
	protected override void UpdateAnchors(bool updateEyeAnchors, bool updateHandAnchors)
	{
		...
		if (updateEyeAnchors && HandView.IsMine)
		{
			if (hmdPresent)
			{
		...
		if (updateHandAnchors && HandView.IsMine)
		{
			...
			Debug.Log("Рука левая позиция" + leftHandAnchor.localPosition);
			Debug.Log("Рука правая позиция" + rightHandAnchor.localPosition);
			Debug.Log("Рука левая поворот" + leftHandAnchor.localRotation);
			Debug.Log("Рука правая поворот" + rightHandAnchor.localRotation);
	...
	public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
	{
		if (stream.IsWriting)
		{
			stream.SendNext(rightControllerAnchor.localPosition);
			stream.SendNext(rightControllerAnchor.localRotation);
			stream.SendNext(leftControllerAnchor.localPosition);
			stream.SendNext(leftControllerAnchor.localRotation);
			stream.SendNext(centerEyeAnchor.localPosition);
			stream.SendNext(centerEyeAnchor.localRotation);
		}
		else
		{
			rightControllerAnchor.localPosition = (Vector3)stream.ReceiveNext();
			rightControllerAnchor.localRotation = (Quaternion)stream.ReceiveNext();
			leftControllerAnchor.localPosition = (Vector3)stream.ReceiveNext();
			leftControllerAnchor.localRotation = (Quaternion)stream.ReceiveNext();
			centerEyeAnchor.localPosition = (Vector3)stream.ReceiveNext();
			centerEyeAnchor.localRotation = (Quaternion)stream.ReceiveNext();
		}
	}

	public void GetOpenVRControllerOffset(Node hand, PhotonView MyPhoton, ref OVRPose GettingHand)
	{
		OVRPose poseOffset = OVRPose.identity;
		if ((hand == Node.LeftHand || hand == Node.RightHand) && OVRManager.loadedXRDevice == OVRManager.XRDevice.OpenVR)
		{
			...
			{
				Vector3 offsetOrientation = (hand == Node.LeftHand) ? OVRManager.OpenVRTouchRotationOffsetEulerLeft : OVRManager.OpenVRTouchRotationOffsetEulerRight;
				poseOffset.orientation = Quaternion.Euler(offsetOrientation.x, offsetOrientation.y, offsetOrientation.z);
				poseOffset.position = (hand == Node.LeftHand) ? OVRManager.OpenVRTouchPositionOffsetLeft : OVRManager.OpenVRTouchPositionOffsetRight;
			}
		}
		GettingHand = poseOffset;
	}
}
